...
import {
  ...
  CalendarDays, ShoppingBag, LogOut, Menu, X,
} from 'lucide-react';

const navItems = [
  { to: '/client/dashboard',    icon: LayoutDashboard,  label: 'Inicio'        },
  { to: '/client/routines',     icon: ClipboardList,    label: 'Mis Rutinas'   },
  { to: '/client/nutrition',    icon: UtensilsCrossed,  label: 'Alimentación'  },
  { to: '/client/appointments', icon: CalendarDays,     label: 'Citas'         },
    ...
    <div className="flex h-screen bg-background overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        <header className="flex items-center h-12 px-4 border-b border-border bg-card shrink-0">
          <button className="lg:hidden p-1.5 text-muted-foreground" onClick={() => setSidebarOpen(true)}>
